...
describe("Attack", () => {
  it("Should be able to read the private variables password and username", async () => {
    ...
    const slot0Bytes = await hre.ethers.provider.getStorage(loginContract.target, 0);

    // Get the storage slot 1
    const slot1Bytes = await hre.ethers.provider.getStorage(loginContract.target, 1);
